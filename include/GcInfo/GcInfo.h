...
enum AllocaFlags {
  Alloca_None = 0x0,
  Alloca_GcPointer = 0x1,
  Alloca_GcAggregate = 0x2,
  Alloca_Pinned = 0x4,
  Alloca_GsCookie = 0x8,
  Alloca_SecurityObject = 0x10,
  Alloca_GenericsContext = 0x20
};
...
struct AllocaInfo {
  ...
  bool isGcPointer() const { return ((Flags & Alloca_GcPointer) != 0); }
  bool isGcAggregate() const { return ((Flags & Alloca_GcAggregate) != 0); }
  bool isGcValue() const {
    return ((Flags & (Alloca_GcPointer | Alloca_GcAggregate)) != 0);
  }
  bool isPinned() const { return ((Flags & Alloca_Pinned) != 0); }
};
...
private:
  ...
  void markGcAlloca(const llvm::AllocaInst *Alloca, const AllocaFlags Flags);

  // Mark additional annotations on a recorded non-GC-Value
  void markIntAlloca(const llvm::AllocaInst *Alloca, const AllocaFlags Flags);
};
...
private:
  ...
  bool isTrackedSlot(const GcSlotId SlotID);
  GcSlotId getSlot(const int32_t Offset, const GcSlotFlags Flags);
  GcSlotId getTrackedSlot(const int32_t Offset);
  GcSlotId getUntrackedSlot(const int32_t Offset, bool IsPinned = false,
