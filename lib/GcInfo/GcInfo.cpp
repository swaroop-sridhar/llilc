...
bool GcInfo::isGcAllocation(const llvm::Value *Value) {
  ...
  if ((Alloca != nullptr) && GcInfo::isGcType(Alloca->getAllocatedType())) {
    return true;
  }
...
void GcFuncInfo::recordAlloca(const AllocaInst *Alloca) {
  ...
  AllocaFlags Flags = AllocaFlags::Alloca_None;

  if (GcInfo::isGcPointer(Type)) {
    Flags = AllocaFlags::Alloca_GcPointer;
  } else if (GcInfo::isGcAggregate(Type)) {
    Flags = AllocaFlags::Alloca_GcAggregate;
  }
...
void GcFuncInfo::markIntAlloca(const AllocaInst *Alloca,
                               const AllocaFlags Flags) {
...
void GcFuncInfo::recordGsCookie(const AllocaInst *Alloca,
  ...
  recordAlloca(Alloca);
  markIntAlloca(Alloca, AllocaFlags::Alloca_SecurityObject);
...
const char *AllocaInfo::getAllocTypeString() const {
  if (Flags & AllocaFlags::Alloca_GsCookie) {
    return "GS Cookie";
  }

  if (Flags & AllocaFlags::Alloca_GenericsContext) {
    if (Flags & AllocaFlags::Alloca_GcPointer) {
  ...
  if (Flags & AllocaFlags::Alloca_SecurityObject) {
  ...
  if (Flags & AllocaFlags::Alloca_GcAggregate) {
    return "GcAggregate";
  }
...
void GcInfoEmitter::encodeUntrackedPointers(const GcFuncInfo *GcFuncInfo) {
  for (auto AllocaIterator : GcFuncInfo->AllocaMap) {
    ...
    if (AllocaInfo.Flags & AllocaFlags::Alloca_GsCookie) {
    ...
    if (AllocaInfo.Flags & AllocaFlags::Alloca_GenericsContext) {
    ...
    if (AllocaInfo.Flags & AllocaFlags::Alloca_SecurityObject) {
...
void GcInfoEmitter::encodeGcAggregate(const AllocaInst *Alloca,
  ...
  bool IsObjectReferece = true; // Only ObjectReferences can be stored in
                                // GC Aggregates.

  assert(GcPtrOffsets.size() > 0 && "GC Aggregate without GC pointers!");

  for (uint32_t GcPtrOffset : GcPtrOffsets) {
    getUntrackedSlot(AggregateOffset + GcPtrOffset, IsPinned, IsObjectReferece);
  }
}
